...
def TraerDeMiniOEntrenamientos():
    ...
    # Listo para entrenar 
    datos = db.query(Entrenamiento).all()
...
# Aca tiene que estar el codigo para pedirle a la DB el ultimo modelo disponible

MODEL_PATH = "app/fire_model.pth"
...
def get_pending(db: Session):
    return db.query(Orden).filter(Orden.status == "Lista para predecir..").first()
    ...
    while True:
        ...
        if orden:
            ...
            try:
                ...
                orden.status = "Error al predecir"
                print(f"Error: {e}")
            db.commit()
        db.close()
        time.sleep(5)
